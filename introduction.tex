...
\title{An Introduction to Molecular Response Properties}
...
\chapter{Introduction and Theoretical Background\label{ch:introduction}}
...
\section{Terms and Fundamental Definitions\label{sec:terms-and-fundamental-definitions}}
...
\section[Macroscopic and Microscopic Response Connections]{Connection Between Macroscopic and Microscopic Properties\label{sec:connection-between-macroscopic-and-microscopic}}
...
\section{Static (time-independent) response properties\label{sec:static-properties}}
...
\subsection{Phenomenological approach\label{ssec:phenomenological-approach}}
...
\subsection{\texorpdfstring{\href{https://chemistry.stackexchange.com/q/74683/194}{\color{black}{Series expansion}}}{Series expansion}\label{ssec:series-expansion}}
...
% \end{equation}
\subsection{Derivative evaluation\label{ssec:derivative-evaluation}}
...
\begin{table}[htbp]
  \centering
  \caption[Possible analytic and numerical permutations for Raman intensities]{Possible permutations of analytic (a) and numerical (n) differentiation for each perturbation term in calculating Raman intensities. The two cancelled entries are not unique due to permutational symmetry.\label{tab:raman-unique-derivatives}}
  \begin{tabular}{ccc}
    \toprule
    \(X_{A}\) & \(F_{i}\) & \(F_{j}\) \\
    \midrule
    a & a & a \\
    a & a & n \\
    \tabnode{a} & \tabnode{n} & \tabnode{a} \\
    n & a & a \\
    a & n & n \\
    n & a & n \\
    \tabnode{n} & \tabnode{n} & \tabnode{a} \\
    n & n & n \\
    \bottomrule
  \end{tabular}
\end{table}
\begin{tikzpicture}[overlay]
  % Define the lines as paths
  \draw [->,red,thick] (1.south west) -- (2.center) -- (3.north east);
  \draw [->,red,thick] (4.south west) -- (5.center) -- (6.north east);
\end{tikzpicture}
...
\subsection{\texorpdfstring{\href{https://chemistry.stackexchange.com/q/80335/194}{\color{black}{Finite difference for numerical derivatives}}}{Finite difference for numerical derivatives}\label{ssec:finite-difference-for-numerical-derivatives}}
...
\subsection{\texorpdfstring{\href{https://chemistry.stackexchange.com/q/89831/194}{\color{black}{Analytic derivative theory}}}{Analytic derivative theory}\label{ssec:analytic-derivative-theory}}
...
\end{equation}
where the index \(m\) runs over all occupied and unoccupied/virtual MOs. Although \(X_{A}\) is being used for the perturbation, all parts of this derivation hold for any general perturbation. The key insight is that the effect of a perturbation on the MO coefficients can be written as the contraction of the unmodified MO coefficients with a unitary matrix describing single-particle excitations from occupied to virtual MOs, as well as deexcitations from virtual to occupied MOs. In matrix form, this is
\begin{equation}
...
\subsection{Perturbation theory\label{ssec:perturbation-theory}}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
In the case where \(\hat{V}\) is an operator with multiple components, such as the dipole operator in~\eqref{eq:dipole-operator}, the energy becomes a rank-2 tensor, the polarizability matrix:
\begin{equation}
...
\section{Dynamic (frequency-dependent) response properties\label{sec:dynamic-properties}}
...
% \section{The coupled-perturbed equations}
